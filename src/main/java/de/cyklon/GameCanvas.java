...
public class GameCanvas extends UICanvas {
    ...
    private static final int spawnValue = 2;
    private static final long animationDelay = 25;
    ...
    public GameCanvas(App app) {
        setBgColor(new Color(186, 172, 159));
        int i1 = 0;
        for (int x = 50; x < (80+50)*blocks.length; x+=80+50) {
            int i2 = 0;
            for (int y = 50; y < (80+50)*blocks[0].length; y+=80+50)
                addComponents(blocks[i1][i2++] = new Block(0, x, y, 80, 80));
            i1++;
        }

        UILabel scoreLabel = new UILabel(50, 25);
        addComponents(scoreLabel);
    ...
    private void spawn(App app) {
        ...
        do {
            x = random.nextInt(blocks.length);
            y = random.nextInt(blocks[0].length);
        } while (blocks[x][y].getValue()!=0);

        blocks[x][y].setValue(spawnValue);
    ...
    private synchronized void moveRight(App app, boolean initial) {
        moving = true;
        Runnable runnable = () -> {
            boolean moved = false;
                    ...
                    for (int y = 0; y < blocks[x].length; y++) {
                        if (blocks[x][y].getValue()!=0) {
                            if (blocks[x+1][y].getValue()==0) {
                                ...
                                moved = true;
                            } else if (blocks[x+1][y].getValue()==blocks[x][y].getValue() && !blocks[x+1][y].isBlocked()) {
                                blocks[x+1][y].setValue(blocks[x][y].getValue()*2);
                                blocks[x][y].setValue(0);
                                score+=blocks[x+1][y].getValue();
                                blocks[x+1][y].setBlocked(true);
                                merged = true;
                            }
                        }
                    }
    ...
    private synchronized void moveLeft(App app, boolean initial) {
        moving = true;
        Runnable runnable = () -> {
            boolean moved = false;
            boolean merged = false;
    ...
    private synchronized void moveDown(App app, boolean initial) {
        moving = true;
        Runnable runnable = () -> {
            boolean moved = false;
            boolean merged = false;
                    ...
                    if (y != block.length - 1) {
                        if (block[y].getValue() != 0) {
                            if (block[y + 1].getValue() == 0) {
                                ...
                                block[y].setValue(0);
                                moved = true;
                            } else if (block[y + 1].getValue() == block[y].getValue() && !block[y+1].isBlocked()) {
                                block[y + 1].setValue(block[y].getValue() * 2);
                                block[y].setValue(0);
                                score+=block[y+1].getValue();
                                block[y+1].setBlocked(true);
                                merged = true;
                            }
                        }
                    }
    ...
    private synchronized void moveUp(App app, boolean initial) {
        moving = true;
        Runnable runnable = () -> {
            boolean moved = false;
            boolean merged = false;
